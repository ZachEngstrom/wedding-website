...
$guestbookResults = $wpdb->get_results( "SELECT * FROM $guestbook_table_name ORDER BY `unread` DESC, `date`");
							...
							echo '
								<tr id="user-'.$i.'" class="'.$unreadClass.'">
									<td class="username column-username has-row-actions column-primary" data-colname="Name">
										<img alt="" src="'.esc_url( get_avatar_url( $guestbookResult->email ) ).'" srcset="'.esc_url( get_avatar_url( $guestbookResult->email ) ).' 2x" class="avatar avatar-32 photo" height="32" width="32"><strong>'.$guestbookResult->firstName.' '.$guestbookResult->lastName.'</strong><br>
									...
									<td class="posts column-posts" data-colname="Message">'.substr($guestbookResult->message,0,160).'...</td>
									<td class="posts column-posts" data-colname="View"><a href="'.get_admin_url().'admin.php?page=match-guestbook-form.php&view-deleted='.$guestbookResult->id.'" class="button button-primary"><span class="dashicons dashicons-visibility"></span></a></td>
								</tr>
							';
						}
					}
